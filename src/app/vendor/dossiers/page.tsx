...
function DossierStatusBadge({ status }: { status: DossierStatus }) {
  const styles: Record<DossierStatus, string> = {
    'Inkoopopdracht_Verstuurd': 'bg-indigo-100 text-indigo-700',
...
export default function VendorDossiersPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h2 className="text-2xl font-bold text-slate-900">Mijn Samenwerkingen</h2>
            ...
            <tbody className="divide-y divide-slate-100">
              {mockDossiers.filter(d => d.relationId === 'R-001').map(dossier => (
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <div>
                      <p className="text-sm font-semibold text-slate-800 group-hover:text-indigo-700 transition-colors truncate max-w-xs">
