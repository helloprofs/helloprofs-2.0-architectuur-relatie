  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-500 mt-1">Alle actieve en afgeronde projecten waar u als opdrachtnemer bij betrokken bent.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        {mockProjects.map((project) => {
          ...
          return (
            <div key={project.id} className="bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow overflow-hidden text-slate-800">
              {/* Card Header */}
              ...
              <div className="px-6 py-4 space-y-4">
                ...
                <div>
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">
                    Mijn Inkoopopdrachten ({purchaseOrders.length})
                  </p>
                    ...
                    <div className="space-y-1.5">
                      {purchaseOrders.map(po => (
                        <div key={po.id} className="flex items-center justify-between bg-slate-50 rounded-lg px-3 py-2 border border-slate-100">
                          ...
                          <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${po.status === 'Verstuurd' ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-100 text-slate-500'
                            }`}>
                            {po.status}
                          </span>
                        </div>
              ...
              <div className="px-6 py-3 border-t border-slate-100 bg-slate-50">
                <Link
                  href={`/vendor/purchase-orders?projectId=${project.id}`}
