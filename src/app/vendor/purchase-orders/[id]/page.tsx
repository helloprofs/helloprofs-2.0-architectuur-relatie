...
// Reuse the badge component for consistency
...
export default async function VendorPurchaseOrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const po = mockPurchaseOrders.find(p => p.id === id);
  if (!po) return notFound();

  const project = mockProjects.find(p => p.id === po.projectId);
  // Filtering dossiers for this PO and for Jan de Bouwer (R-001)
  const dossiers = mockDossiers.filter(d => d.purchaseOrderId === po.id && d.relationId === 'R-001');
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <ArrowLeft size={15} />
        Terug naar Mijn Inkoopopdrachten
      </Link>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 text-slate-800">
        <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
          <div>
            ...
            <h2 className="text-2xl font-bold text-slate-900">{po.title}</h2>
            {project && (
          ...
          <span className={`px-3 py-1.5 rounded-full text-sm font-semibold flex-shrink-0 ${
            po.status === 'Verstuurd' ? 'bg-indigo-100 text-indigo-700' :
        ...
        <div className="flex flex-wrap gap-6 mt-4 pt-4 border-t border-slate-100">
          <div className="flex items-center gap-2 text-sm text-slate-500">
            ...
            Ontvangen op: <strong className="text-slate-700">{new Date(po.dateCreated).toLocaleDateString('nl-NL')}</strong>
          </div>
          <div className="flex items-center gap-2 text-sm text-slate-500">
            <FileText size={14} className="text-slate-400" />
            <strong className="text-slate-700">{dossiers.length}</strong> actief dossier
          </div>
        </div>
      </div>

      {/* Dossier (Vendor's perspective) */}
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden text-slate-800">
        <div className="px-6 py-4 border-b border-slate-100">
          <h3 className="font-semibold text-slate-900">Mijn Dossier</h3>
          <p className="text-xs text-slate-500 mt-0.5">Uw specifieke dossier voor deze inkoopopdracht.</p>
        </div>

        {dossiers.length === 0 ? (
          <div className="px-6 py-12 text-center">
            <p className="text-sm text-slate-400 italic">Geen dossier gevonden voor deze opdracht.</p>
          </div>
        ) : (
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-slate-100">
                <th className="px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Dossiernummer</th>
              ...
              {dossiers.map(dossier => (
                ...
                >
                  <td className="px-6 py-4">
                    <Link href={`/vendor/dossiers/${dossier.id}`} className="text-sm font-mono font-bold text-slate-700 group-hover:text-indigo-600 transition-colors hover:underline">
                      {dossier.id}
                    </Link>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-center">
                    <button className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-100 text-slate-600 text-xs font-semibold hover:bg-indigo-100 hover:text-indigo-700 transition-colors">
                      <History size={13} />
                      {dossier.historyCount} stappen
                    </button>
                  </td>
                  <td className="px-6 py-4 text-right">
                    <Link href={`/vendor/dossiers/${dossier.id}`} className="inline-flex items-center gap-1 text-xs font-semibold text-indigo-600 group-hover:text-indigo-800 transition-colors">
