...
function StatusBadge({ status }: { status: RelationStatus }) {
  const styles: Record<RelationStatus, string> = {
    ...
    'Aangemeld': 'bg-indigo-100 text-indigo-700',
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-900">Mijn Relaties</h2>
          <p className="text-slate-500 mt-1">Beheer uw connecties met opdrachtgevers en partners.</p>
        </div>
        <button className="flex items-center gap-2 bg-indigo-800 hover:bg-indigo-700 text-white px-4 py-2.5 rounded-lg text-sm font-semibold transition-colors">
          <Plus size={16} />
                ...
                <tr key={relation.id} className="hover:bg-slate-50 transition-colors cursor-pointer group">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center flex-shrink-0 group-hover:border-indigo-200 transition-colors">
                        {relation.type === 'ZZP' ? (
                      ...
                      <div>
                        <p className="text-sm font-semibold text-slate-900 group-hover:text-indigo-700 transition-colors">{relation.name}</p>
